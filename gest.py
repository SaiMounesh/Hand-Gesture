...
# Make Detector
detector = HandDetector(detectionCon=0.10)
StartDist = None
scale = 0
cx, cy = 500, 250

# Load Image
img1 = cv2.imread('IMG_3079.png')
if img1 is None:
    raise ValueError("Error: 'IMG_3079.png' not found or could not be loaded.")

fourcc = cv2.VideoWriter_fourcc(*'MP4V')  # Codec, can use 'XVID', 'MJPG', 'MP4V', etc.
out = cv2.VideoWriter('output_video2.mp4', fourcc, 20.0, (1280, 720))

initial_width = 300
aspect_ratio = img1.shape[1] / img1.shape[0]  # Aspect ratio (width / height)
initial_height = int(initial_width / aspect_ratio)

img1 = cv2.resize(img1, (initial_width, initial_height))
